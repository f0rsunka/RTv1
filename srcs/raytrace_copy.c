...
t_color		trace_ray(t_vec3 camera, t_vec3 dir, t_rtv *r)
{
	...
	while (i < r->count_objects)
	{
		...
		if (intersect_ray_sphere(camera, dir, r->sphere[i], &closest.dist))
		{
			...
			count_light = calculate_lightning(r, reverse_dir, normal_dir, r->sphere[i].specular);
			// count_light = calculate_lightning(r, dir, normal_dir, r->sphere[i].specular);
			total_color = add_light(r->sphere[i].color, count_light);
			return (total_color);
		}
		i++;
	}
